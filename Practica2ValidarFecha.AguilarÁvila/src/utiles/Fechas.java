...
public class Fechas {
	...
	/**
	 * Devuelve el numero de dias que tiene un mes de determinado anyo. El anyo es
	 * necesario porque el mes de febrero puede tener un numero distinto de dias
	 * dependiendo de si el anyo es o no bisiesto.
	 * 
	...
	public static int extraerDia(String fecha) {
		int posicion = 1;
		return extraerFecha(fecha, posicion);
	}
	...
	/**
	 * Dada una fecha expresada con tres valores (dia, mes y anyo), devuelve si la
	 ...
	 * que tiene el mes - El mes tiene que estar entre 1 y 12 - El anyo tiene que
	 * ser positivo.
	 * 
	...
	public static boolean esFechaValida(int dia, int mes, int anyo) {
		...
		boolean fechaValida;
		if (dia <= diasDelMes(mes, anyo) && mes <= 12 && anyo > 0) {
	 ...
	 * @return un entero que ser�: -1 si la primera fecha es menor (anterior) que la
	 *         segunda 0 si las dos fechas son iguales +1 si la primera fecha es
	 *         mayor (posterior) que la segunda
	 */
	...
	public static int extraerFecha(String restoFecha, int posicion) {
		...
		for (int i = 0; i < posicion; i++) {
			if (restoFecha.indexOf('/') != -1) {
				pFecha = restoFecha.substring(0, restoFecha.indexOf('/'));
			} else {
				pFecha = restoFecha.substring(0);
			}
